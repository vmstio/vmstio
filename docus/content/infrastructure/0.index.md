...
(More information on that original configuration [can be found here](https://write.vmst.io/vmstan/infrastructure-as-of-november-28).)

Over [many](https://write.vmst.io/vmstan/infrastructure-as-of-december-15) [iterations](https://write.vmst.io/vmstan/infrastructure-as-of-jan-2023), we eventually moved Redis to a managed instance, separated Elastic Search to its own host, and got better about controlling Sidekiq queues.
...
Nginx is running in Docker containers using the [latest Alpine slim image](https://hub.docker.com/_/nginx/tags?page=1&name=alpine).
Currently this is version 1.24.

A reverse proxy is a server that sits between client devices and web servers, acting as an intermediary in the communication process. It receives requests from clients, processes them, and forwards them to the appropriate backend server. Once the backend server responds, the reverse proxy forwards the response back to the client.
...
There has been discussion within the Mastodon project of replacing hiredis with the native [redis-rb](https://github.com/redis/redis-rb) driver, as the hiredis code has not been updated in over 4 years.
Using the native redis-rb driver provides support for TLS connections.

To accommodate this requirement in the past we have used [HAProxy](https://www.haproxy.org) or [Stunnel](https://www.stunnel.org) to take the un-encrypted connection requests and encrypt those connections between the Mastodon components and Redis.

We now remove the hiredis driver from our installation and use redis-rb instead.
...
- One virtual machine ([Scotty](https://memory-alpha.fandom.com/wiki/Nyota_Uhura)) with 1 vCPU and 2 GB of memory.
...
WriteFreely runs across both of the core Kirk and Spock servers.
...
Matrix runs on our Scotty server.
